...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    api.get('/homepage').then(res => {
      ...
      const ann = sections.find(s => s.type === 'announcement');
      setAnnouncement(ann?.title || '');
    }).catch(() => {});
  }, []);

  useEffect(() => { setMenuOpen(false); setDropdownOpen(false); setSearchOpen(''); }, [location]);
    ...
    <>
        ...
        <div className="nav-container">
          ...
          <div className="nav-bottom-row">
            ...
            {categories.slice(0, 6).map(cat => (
              <Link key={cat} to={`/products?category=${encodeURIComponent(cat)}`} className="nav-link">{cat}</Link>
            ))}
          </div>
        </div>
      </nav>
      ...
      <div className={`mobile-menu ${menuOpen ? 'open' : ''}`}>
        ...
        <div className="mobile-menu-body">
          ...
          {categories.map(cat => (
            <Link key={cat} to={`/products?category=${encodeURIComponent(cat)}`} className="mobile-link" onClick={() => setMenuOpen(false)}>{cat}</Link>
          ))}
